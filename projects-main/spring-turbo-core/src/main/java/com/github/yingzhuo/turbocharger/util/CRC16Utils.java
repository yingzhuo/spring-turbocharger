...
public final class CRC16Utils {
	 ...
	 * @return 累加和
	 */
	public static int crc16Value(byte[] data) {
		return crc16Value(data, 0, data.length);
	}

	private static int crc16Value(byte[] bytes, int off, int len) {
		int crc = 0x0000;
		int end = off + len;
	 ...
	 * @return 累加和 (字符串表示)
	 */
	public static String crc316Hex(byte[] data) {
		var value = crc16Value(data);
